...
import {
  Clock, Play, Archive, CheckCircle, XCircle, AlertCircle,
  Plus, Pencil, Trash2, CalendarClock, Loader2, ScrollText, X,
} from 'lucide-react'
...
export function TasksTab() {
  ...
  const handleTrigger = async (id: number) => {
    setTriggering(id)
    try { await window.electronAPI.triggerSchedule(id) }
    finally { setTriggering(null); loadData() }
  }

  const openNew = () => { setForm(EMPTY_TASK_FORM); setFormError(null); setEditingTaskId(null); setShowForm(true) }
  const openEdit = (t: PendingTask) => {
    const hasTime = t.run_at !== null
    setForm({ name: t.name, prompt: t.prompt, timing: hasTime ? 'later' : 'now', runAt: hasTime ? new Date(t.run_at!).toISOString().slice(0, 16) : '' })
    setFormError(null); setEditingTaskId(t.id); setShowForm(true)
  }
  const cancelForm = () => { setShowForm(false); setEditingTaskId(null); setFormError(null) }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setShowForm(false); setEditingTaskId(null); await loadData()
    } finally { setSaving(false) }
  }

  const handleDeletePending = async (id: number) => {
    const res = await window.electronAPI.deleteTask(id)
    if (res.success) { setDeletingId(null); await loadData() }
  }
  ...
  const formatRunAt = (runAt: number | null) => runAt === null ? 'Immediately' : new Date(runAt).toLocaleString()
  ...
  const handleViewSystemPrompt = async (name: string, prompt: string) => {
    ...
    try {
      const result = await window.electronAPI.getTaskSystemPrompt(name, prompt)
  ...
  const handleViewScheduleSystemPrompt = async (name: string, prompt: string) => {
    setSystemPromptLoading(true)
    try {
          ...
          <div className="relative bg-parchment-card rounded-2xl shadow-2xl w-full max-w-lg max-h-[70vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-parchment-dark">
              <div>
                ...
                <p className="text-xs text-ink-muted mt-0.5">Task Configuration</p>
              </div>
              <button
      ...
      {showForm && (
        <section>
          <div className="bg-parchment-card border border-accent/30 rounded-xl p-6 space-y-4">
            <h3 className="font-medium text-ink-primary text-sm">{editingTaskId !== null ? 'Edit Task' : 'New Task'}</h3>
            <div className="space-y-3">
              <div>
                <label className="block text-xs text-ink-muted mb-1">Name</label>
                <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Summarise this week's notes" className="w-full bg-parchment-sidebar border border-parchment-dark rounded-lg px-3 py-2 text-sm text-ink-primary placeholder:text-ink-light focus:outline-none focus:border-accent" />
              </div>
              <div>
                <label className="block text-xs text-ink-muted mb-1">Prompt</label>
                <textarea value={form.prompt} onChange={e => setForm(f => ({ ...f, prompt: e.target.value }))} placeholder="Describe what the agent should do..." rows={4} className="w-full bg-parchment-sidebar border border-parchment-dark rounded-lg px-3 py-2 text-sm text-ink-primary placeholder:text-ink-light focus:outline-none focus:border-accent resize-none" />
              </div>
              <div>
                <label className="block text-xs text-ink-muted mb-2">When to run</label>
                <div className="flex gap-2">
                  <button onClick={() => setForm(f => ({ ...f, timing: 'now' }))} className={`flex-1 py-2 text-sm rounded-lg border transition-colors ${form.timing === 'now' ? 'border-accent bg-accent/10 text-accent' : 'border-parchment-dark text-ink-muted hover:border-accent/50'}`}>Immediately</button>
                  <button onClick={() => setForm(f => ({ ...f, timing: 'later' }))} className={`flex-1 py-2 text-sm rounded-lg border transition-colors ${form.timing === 'later' ? 'border-accent bg-accent/10 text-accent' : 'border-parchment-dark text-ink-muted hover:border-accent/50'}`}>At a specific time</button>
                </div>
                {form.timing === 'later' && (
                  <input type="datetime-local" value={form.runAt} onChange={e => setForm(f => ({ ...f, runAt: e.target.value }))} min={new Date().toISOString().slice(0, 16)} className="mt-2 w-full bg-parchment-sidebar border border-parchment-dark rounded-lg px-3 py-2 text-sm text-ink-primary focus:outline-none focus:border-accent" />
                )}
              </div>
            </div>
                ...
                {deletingId === t.id ? (
                  <div className="p-5 flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => handleDeletePending(t.id)} className="px-3 py-1.5 text-xs bg-red-500 hover:bg-red-600 text-white rounded-lg transition-colors">Delete</button>
                    </div>
                  </div>
                ) : (
                  <div className="p-5 flex items-start justify-between gap-4">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <button onClick={() => handleViewSystemPrompt(t.name, t.prompt)} disabled={systemPromptLoading} title="View system prompt" className="p-1.5 text-ink-muted hover:text-ink-primary disabled:opacity-40 transition-colors"><ScrollText className="w-3.5 h-3.5" /></button>
                      <button onClick={() => openEdit(t)} disabled={showForm} className="p-1.5 text-ink-muted hover:text-ink-primary disabled:opacity-40 transition-colors"><Pencil className="w-3.5 h-3.5" /></button>
                      <button onClick={() => setDeletingId(t.id)} disabled={showForm} className="p-1.5 text-ink-muted hover:text-red-500 disabled:opacity-40 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="space-y-3">
            {activeSchedules.map(s => (
              <div key={s.id} className="bg-parchment-card rounded-xl p-5 border border-parchment-dark flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => handleViewScheduleSystemPrompt(s.name, s.prompt)} disabled={systemPromptLoading} title="View system prompt" className="p-2 text-ink-muted hover:text-ink-primary disabled:opacity-40 transition-colors">
                    <ScrollText className="w-4 h-4" />
                  </button>
                  <button onClick={() => handleTrigger(s.id)} disabled={!!liveTask || triggering !== null} className="flex items-center gap-2 px-4 py-2 bg-accent hover:bg-accent-hover disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg text-sm font-medium transition-colors">
...
// keep unused import satisfied for linting
void Clock
